...
public class GamePanel extends JPanel implements KeyListener{
    ...
    private GamePanelTheme theme;
    
    public GamePanel(Game frame){
    	this.frame = frame;
    	theme = new GamePanelTheme();
    	//TODO defaultTheme
    	theme.setTheme("");
    ...
    private void initComponents() {
        ...
        BackgroundPanel.setBackground(theme.backgroundColor);
        BackgroundPanel.setLayout(null);

        TopPanel.setBackground(theme.topPanelColor);
        TopPanel.setPreferredSize(new java.awt.Dimension(1600, 80));

        HPbar.setBackground(theme.hpBarBackgroundColor);
        HPbar.setFont(new java.awt.Font("Segoe UI Black", 1, 18)); // NOI18N
        HPbar.setForeground(theme.hpBarForegroundColor);
        HPbar.setValue(100);
        ...
        ScoreNum.setForeground(theme.scoreNumColor);
        ScoreNum.setText("00");

        ScoreLabel.setFont(new java.awt.Font("Segoe UI Black", 1, 36)); // NOI18N
        ScoreLabel.setForeground(theme.scoreLabelColor);
        ...
        pauseButton.setText("| |");
        pauseButton.setPreferredSize(new java.awt.Dimension(79, 46));
        pauseButton.addActionListener((e)-> {
        	PauseButtonActionPerformed(e);
        });
        
        showStage = new JLabel();
        showStage.setForeground(theme.showStageColor);
        showStage.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        showStage.setText("Stage: 1");
        showStage.setHorizontalTextPosition(javax.swing.SwingConstants.LEFT);
        showStage.setInheritsPopupMenu(false);
        ...
        TopPanelLayout.setHorizontalGroup(
            ...
            .addGroup(TopPanelLayout.createSequentialGroup()
                ...
                .addComponent(ScoreNum, javax.swing.GroupLayout.PREFERRED_SIZE, 320, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(44, 44, 44)
                .addGroup(TopPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addComponent(showStage, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        ...
        BottomPanel.setBackground(theme.bottomPanelColor);
        ...
        DefaultBombSkillColor = BombSkillButton.getBackground();
        DefaultHealSkillColor = HealSkillButton.getBackground();
        DefaultFreezeSkillColor = FreezeSkillButton.getBackground();
    }// </editor-fold>  
    ...
    public void PauseButton(boolean isPaused) {
    	pauseButton.setSelected(isPaused);
    	if (pauseButton.isSelected()) {
    		setButtonState(false);
    		for (Enemy e : gameLogic.enemies) {e.pauseState(true);}
    		
    		pauseButton.setText(">");
    		gameLogic.gamePause();
    	} else {
    		setButtonState(true);
    		for (Enemy e : gameLogic.enemies) {e.pauseState(false);}
    ...
    private void PauseButtonActionPerformed(java.awt.event.ActionEvent evt) {
    	PauseButton(pauseButton.isSelected());
    }

    
    
    // Variables declaration - do not modify
    public JLabel showStage;
    ...
    private Color DefaultBombSkillColor ;
    private Color DefaultFreezeSkillColor ;
    private Color DefaultHealSkillColor ;
    
    public void resetSkillButton() {
    	BombSkillButton.setSelected(true);
    	HealSkillButton.setSelected(true);
    	FreezeSkillButton.setSelected(true);
    	cooldown = new int[3];
    	
    	if (BombTimer != null) {
    		BombSkillButton.setText("Bomb");
    		BombSkillButton.setBackground(DefaultBombSkillColor);
    		BombTimer = new Timer(10, null);
    		//BombTimer.isRunning() &&
		}
	    if (HealTimer != null) {
	    	HealSkillButton.setText("Heal");
    		HealSkillButton.setBackground(DefaultHealSkillColor);
    		HealTimer = new Timer(10, null);
    		//HealTimer.isRunning() && 
		}
	    if (FreezeTimer != null) {
	    	FreezeSkillButton.setText("Freeze");
    		FreezeSkillButton.setBackground(DefaultFreezeSkillColor);
    		FreezeTimer = new Timer(10, null);
    		//FreezeTimer.isRunning() && 
		}
        HealDuration 	= new Timer(10, null);
        FreezeDuration	= new Timer(10, null);

    }
    
    private void setSkillAction() {
    	
//    	bombSkillButtonAction();
//    	healSkillButtonAction();
//    	freezeSkillButtonAction();
    ...
    private void bombSkillButtonAction() {
    	gameLogic.removeAllEnemy(false);
		startCooldown(BombSkillButton, 600, BombTimer, DefaultBombSkillColor);
    }
    
    private void healSkillButtonAction() {
    	//TODO cool_down need to be more than skill duration if not it's going to be broken
		healing(4,15);
		startCooldown(HealSkillButton, 300, HealTimer, DefaultHealSkillColor);
    }
    
    private void healing(int second,int healing) {
    	HPbar.setValue(HPbar.getValue()+healing);
    	long startTime = System.currentTimeMillis();
    	HealDuration = new Timer(500, (e)-> {
    		HPbar.setValue(HPbar.getValue()+healing);
    ...
    private void freezeSkillButtonAction() {
    	//TODO cool_down need to be more than skill duration if not it's going to be broken
		freezing(15);
		startCooldown(FreezeSkillButton, 300, FreezeTimer, DefaultFreezeSkillColor);
    }
    ...
    private void freezing(int second) {
    	...
    	for (Enemy enemy : gameLogic.enemies) {
            ...
            enemy.setBorder(new javax.swing.border.MatteBorder(new javax.swing.ImageIcon(Objects.requireNonNull(getClass().getResource("res/images/freeze.png"))))); // NOI18N
    	}
    	FreezeDuration = new Timer(second*1000, (e)-> {
    		for (Enemy enemy : gameLogic.enemies) {
    			enemy.speed = gameLogic.ingameSpeed;
    			enemy.setDefaultBorder();
    		}
    		FreezeDuration.stop();
    	});
    ...
    private void startCooldown(JButton skillButton, int second, Timer skillTimer, Color skillColor) {
    	int index;
    	...
    	cooldown[index] = second;
    	String temp = skillButton.getText();
    	skillButton.setBackground(skillColor.darker().darker().darker());
    	skillButton.setEnabled(false);
    	skillButton.setText(second+"s");
    	skillTimer = new Timer(1000, (e)-> {
    		cooldown[index]--;
    		skillButton.setText(cooldown[index] + "s");
    		if (cooldown[index] <= 0) {
    			skillButton.setEnabled(true);
        		skillButton.setBackground(skillColor);
        		skillButton.setText(temp);
        		((Timer) e.getSource()).stop();
			}
    	});
    	
    	skillTimer.setRepeats(true);
    	skillTimer.start();
    ...
    private void configureToggleButton(JToggleButton toggleButton, int playerValue) {
    	toggleButton.setBackground(theme.toggleButtonColor);
        toggleButton.addActionListener((e -> {
            toggleBit(toggleButton, playerValue);
        }));
    }
    
    private void toggleBit(JToggleButton toggleButton, int playerValue) {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_Q) {
            numTabPanel.setSelectedComponent(OcPanel);
        } else if (e.getKeyCode() == KeyEvent.VK_W) {
            numTabPanel.setSelectedComponent(DeciPanel);
        } else if (e.getKeyCode() == KeyEvent.VK_E) {
            numTabPanel.setSelectedComponent(HexPanel);
        } else if (e.getKeyCode() == KeyEvent.VK_R) {
        	numTabPanel.setSelectedComponent(SkillPanel);
