...
public class GameLogic implements Runnable{
    ...
    private void stageScaling(boolean reset) {
    	if (reset) {
			...
			//TODO 0.025f too slow? I think
			ingameSpeed = (defaultEnemySpeed*speedMultipier) + scale*0.025f;
			ingameEnemySpawnrate = (defaultEnemySpawnrate) - scale*0.25f;
			System.out.println(ingameSpeed);
			System.out.println(ingameEnemySpawnrate+"\n");
			stageScaling();
		}
    }
        ...
        while (enemyIterator.hasNext()) {
            ...
            if (enemy.x < -155) {
                if (gamePanel.HPbar.getValue()>0){
                    gamePanel.HPbar.setValue(gamePanel.HPbar.getValue()-0);
                    //TODO สูตรอมตะ
                }
                SwingUtilities.invokeLater(() -> gamePanel.BackgroundPanel.remove(enemy));
                enemyIterator.remove();
