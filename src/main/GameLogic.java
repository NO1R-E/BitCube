...
public class GameLogic implements Runnable{
	...
	private int showStage = 1;
    private int currentScore = 0;
    public int currPlayerNumber = 0;
    public GamePanel gamePanel;
    private final int defaultEnemySpawnpoint = 1600;
    private final float defaultEnemySpeed = 0.50f;
    private final float defaultEnemySpawnrate = 400;
    public float ingameEnemySpawnrate;
    public float ingameSpeed;
    public float speedMultiplier = 1; // 1 as default;
    private int scale;
    ...
    public void newGame() {
        ...
        playing = true;
        stageScaling(true);
        gamePanel.PauseButton(false);
        gamePanel.resetSkillButton();
        type = 0;
        currentScore=0;
    ...
    private void stageScaling(boolean reset) {
    	if (reset) {
    		showStage = 1;
    		gamePanel.showStage.setText("Stage: "+showStage);
    		ingameSpeed = defaultEnemySpeed;
    		ingameEnemySpawnrate = defaultEnemySpawnrate;
    		enemyTimer = (int)ingameEnemySpawnrate-1;
    		enemyTracking = 0;
    		switch (difficulty) {
			case "easy":
				scale = 1;
			...
			ingameSpeed = (defaultEnemySpeed*speedMultiplier) + scale*0.025f;
			if (ingameEnemySpawnrate>=10) {
				ingameEnemySpawnrate = (defaultEnemySpawnrate) - scale*0.25f;
			}
			System.out.println("Speed Multiplier: "+speedMultiplier);
			System.out.println("Ingame speed: "+ingameSpeed);
			System.out.println("Ingame spawnrate: "+ingameEnemySpawnrate+"\n---------");
			stageScaling();
		}
    }
    
    
    //TODO idk if this is too slow or fast
    private void stageScaling() {
    	if (enemyTracking >= 5) {
			scale+=5;
			showStage++;
			gamePanel.showStage.setText("Stage: "+showStage);
...
//    	System.out.println(enemyTimer);
        if (enemyTimer >= ingameEnemySpawnrate){
...
//            	System.out.println(type);
            	Enemy newEnemy = new Enemy(defaultEnemySpawnpoint, location, ingameSpeed, SelectionType());
